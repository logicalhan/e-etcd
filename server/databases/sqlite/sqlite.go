...
func newDB(db *sql.DB, dir string) *SqliteDB {
	return &SqliteDB{
		DB:           db,
		Dir:          dir,
...
func (s *SqliteDB) Begin(writable bool) (interfaces.Tx, error) {
	...
	return &SqliteTx{tx: tx, writable: writable, db: s.DB, dir: s.Dir}, nil
}
...
func (s *SqliteDB) Defrag(logger *zap.Logger, dbopts interface{}) error {
	_, err := s.DB.Exec(defrag)
	return err
}
...
func (t *SqliteTx) WriteTo(w io.Writer) (int64, error) {
	//TODO implement me
	panic("implement me")
}
