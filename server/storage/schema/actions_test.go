...
func putKeyValues(tx backend.BatchTx, bucket bucket.Bucket, kvs map[string]string) {
	for k, v := range kvs {
		tx.UnsafePut(bucket, []byte(k), []byte(v))
	}
}

func assertBucketState(t *testing.T, tx backend.BatchTx, bucket bucket.Bucket, expect map[string]string) {
	t.Helper()
